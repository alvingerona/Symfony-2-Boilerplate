...
class Machine
    ...
    PROJECT_PATH  = File.dirname(File.dirname(BASE_PATH))
    ...
    def self.hostNames
        ...
        CONF['network']['environments'].each do |environment|
            CONF['network']['subdomains'].each do |subdomain|
                names << subdomain + (subdomain == '' ? '' : '.') + 'localhost.' + environment + '.' + CONF['network']['domain']
            end
        end
    ...
    def self.MountShared(config)
        %w{ app/cache app/logs web/bundles vendor }.each do |dir|
            config.vm.provision "shell", run: "always" do |s|
                s.inline = "sudo mount -o bind $2/$3/ $1/$3/"
                s.args   = [self.sharedPath, self.tmpPath, dir]
            end
        end
    end
    ...
    def self.updateApp(settings)
        ...
        subSettings['domain']       = CONF['network']['domain']
